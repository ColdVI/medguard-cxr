...
| RSNA grounding | `make eval-grounding-rsna` | `results/grounding_rsna_eval.json`, `results/overlays/rsna/rsna_grid.png` | completed | RSNA test split, cam_threshold=0.80, box_extraction=multi (both val-selected): pneumonia AUROC 0.8277, AUPRC 0.5655, pointing-game 0.4286, mean IoU 0.3139, IoU>=0.5 0.1607, mAP@0.5 0.0091 | Pneumonia-specific RSNA Lung Opacity validation only; no broad localization claim. 3 of 4 localization metrics improved over the val-sweep values at this threshold; only pointing-game is lower (0.5138 to 0.4286), a difference of ~1.8 SE at n=112 and consistent with sampling noise. Threshold was not re-tuned in response. |
...
| Rule-based VQA | tests and `make eval-vlm-zero-shot` | `results/vlm_zero_shot_eval.json` | completed as rule-based baseline | exact match 0.6091, template adherence 1.0, hallucination rate 0.0 | Synthetic template-generated QA set, not real clinical questions; rule-based only, no free-text VLM claim. |
...
| Localization | Pointing game acc | P3 | > 0.50 | 0.4286 on RSNA test split, confidence-gated examples (target not met; 0.5138 was the val-sweep value used for threshold selection; the ~8.5pp gap is ~1.8 SE at n=112, not re-tuned against this test result) |
| Cross-dataset localization | NIH Pneumonia -> RSNA Lung Opacity pointing/IoU | P3 | report only | RSNA test split: pneumonia AUROC 0.8277, AUPRC 0.5655, mean IoU 0.3139 |
