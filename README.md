...
| RSNA grounding | `make eval-grounding-rsna` | `results/grounding_rsna_eval.json`, `results/overlays/rsna/rsna_grid.png` | completed | RSNA test split, cam_threshold=0.80 (val-selected): pneumonia AUROC 0.8277, AUPRC 0.5655, pointing-game 0.4286, mean IoU 0.3059, mAP@0.5 0.0077 | Pneumonia-specific RSNA Lung Opacity validation only; no broad localization claim. Pointing-game is lower on this test read than the 0.5138 seen during val threshold selection; threshold was not re-tuned in response. |
...
| Localization | Pointing game acc | P3 | > 0.50 | 0.4286 on RSNA test split, confidence-gated examples (target not met; 0.5138 was the val-sweep value used for threshold selection, not re-tuned against this test result) |
| Cross-dataset localization | NIH Pneumonia -> RSNA Lung Opacity pointing/IoU | P3 | report only | RSNA test split: pneumonia AUROC 0.8277, AUPRC 0.5655, mean IoU 0.3059 |
